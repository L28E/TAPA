...
% Instead of bisecting the device using logical indexing, pick two points and create a linear
...
Na = 1e16 * 1e6;
NetDoping(x < leftJunc) = Nd;
NetDoping(x >= rightJunc) = -Na;
NetDoping(x >= leftJunc & x < rightJunc) = linspace(Nd,-Na,(rightJunc-leftJunc)/dx);
